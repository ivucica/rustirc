use tokio::io::{AsyncReadExt,AsyncWriteExt}; // adds new methods on TcpStream etc.
use tokio::io::{AsyncBufRead,AsyncBufReadExt}; // allows for conn.lines()
...
async fn chat_conn(mut conn: tokio::net::TcpStream) -> Result<(), String> {
    conn.write_all("Name? ".as_bytes()).await.map_err(|e| e.to_string())?;

    let buffered_reader =  tokio::io::BufReader::new(conn);
    let mut line_reader = buffered_reader.lines();

    let name = line_reader.next_line().await.map_err(|e| e.to_string())?.unwrap(); // TODO: fix panic by using unwrap_or_default() or similar
